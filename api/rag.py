...
async def embed_texts(http: httpx.AsyncClient, texts: List[str]) -> List[List[float]]:
    r = await http.post(f"{settings.ollama_url}/api/embeddings", json={"model": settings.embed_model, "input": texts})
    r.raise_for_status()
    d = r.json()
...
async def upsert_texts(http: httpx.AsyncClient, qc: QdrantClient, texts: List[str], metas: List[dict] | None=None):
    ...
    dim = len(vecs[0])
    try: qc.get_collection(settings.collection)
    except Exception:
        qc.recreate_collection(settings.collection, vectors_config=VectorParams(size=dim, distance=Distance.COSINE))
    points=[]
    for i,(v,t) in enumerate(zip(vecs,texts)):
        meta = {} if not metas or i>=len(metas) or metas[i] is None else metas[i]
        meta = {**meta, "text": t}
        points.append(PointStruct(vector=v, payload=meta))  # omit id => auto
...
async def search_similar(http: httpx.AsyncClient, qc: QdrantClient, query: str, top_k: int) -> List[Tuple[str, float]]:
    ...
    res = qc.search(collection_name=settings.collection, query_vector=qv, limit=top_k, with_payload=True)
